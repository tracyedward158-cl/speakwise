// ⚠️ 本文件是 Web 版 src/utils/exporters.js 的移植副本，有平台接缝，不参与 sync-core 自动同步。
//    与 Web 版的差异只有最后一层「把文件交出去」：
//      Blob + <a download> -> platform/export.js（微信转发面板 / 剪贴板兜底）
//    其余（hydrateRecords / buildExportPayload / buildFilename）逐字未改。
// ── 练习记录导出（JSON）──
//
...
import { exportJsonFile } from "../../platform/export";
...
// 小程序没有「下载到磁盘」，只能走微信转发面板或剪贴板（见 platform/export.js）。
// 原来的 Blob + <a download> + URL.createObjectURL 三段全部不适用。
//
// ⚠️ 「绝不导出 token」这条约束不受影响：payload 是由 buildExportPayload 显式
//    逐字段拷出来的，token 从来不在其中（不是靠过滤，是靠构造）。
export async function downloadJson(filename, payload) {
  return exportJsonFile(filename, payload);
}

/** 一步导出。返回 payload，调用方可以拿 recordCount 报数。 */
/** 一步导出。现在是异步的 —— 调起转发面板 / 写剪贴板都需要 await。返回 payload。 */
export async function exportRecordsJson(records, { scope = "记录", label = "", extra, includeClient } = {}) {
  const payload = buildExportPayload(records, { scope, label, extra, includeClient });
  await downloadJson(buildFilename(scope, label), payload);
  return payload;
}

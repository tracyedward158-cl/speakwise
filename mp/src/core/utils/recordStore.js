// ⚠️ 本文件是 Web 版 src/utils/recordStore.js 的移植副本，有平台接缝，不参与 sync-core 自动同步。
//    与 Web 版的差异只有三处，都已在下方标注 [移植]：
//      1. localStorage -> platform/storage（语义逐位兼容的垫片）
//      2. getRecommendedExercises 的 target 改成逻辑名，页面侧再解析成小程序路径
//      3. 单键 1MB 上限（小程序）低于 localStorage 的 ~5MB，见 storage.js 注释
...
export function getOwnerId() {
  const owned = storage.getItem(OWNER_KEY);
  return owned !== null ? owned : getStudentId();
}

export function setOwnerId(id) {
  if (id == null) storage.removeItem(OWNER_KEY);
  else storage.setItem(OWNER_KEY, String(id));
}
...
function writeDrafts(list) {
  try {
    storage.setItem(DRAFT_KEY, JSON.stringify(list.slice(-MAX_DRAFTS)));
  } catch (e) {
    // 配额超限不该打断对话
...
export function getStudentProfile() {
  return {
    ...
    hsk: storage.getItem(HSK_KEY) || "未知",
  };
}

export function setStudentNickname(name) {
  storage.setItem(NICKNAME_KEY, name.trim().slice(0, 12));
}

export function setStudentHsk(level) {
  storage.setItem(HSK_KEY, level);
}
